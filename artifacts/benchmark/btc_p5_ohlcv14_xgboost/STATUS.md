...
One P5 cell: the `xgboost` model on the `ohlcv14` information set — the frozen 14-column control, re-run on P5's own sample universe — over
...
**This is the deciding cell's control.** Together with `btc_p5_ohlcv14_plus_mtf_v1_xgboost` it is the one comparison P5's preregistered rule reads; the decision itself is `benchmark/btc_p5_decision`.
