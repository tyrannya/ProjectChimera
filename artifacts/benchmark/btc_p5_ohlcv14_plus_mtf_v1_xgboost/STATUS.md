...
One P5 cell: the `xgboost` model on the `ohlcv14_plus_mtf_v1` information set — both, 42 columns, concatenated in that order — over
...
**This is the deciding cell's combined arm.** Together with `btc_p5_ohlcv14_xgboost` it is the one comparison P5's preregistered rule reads; the decision itself is `benchmark/btc_p5_decision`.
