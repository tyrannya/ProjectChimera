...
def checkpoint_of(cells: Sequence[dict[str, Any]]) -> Checkpoint:
    ...
    Four things are required, not one. The cells must name the same checkpoint;
    that name must be a checkpoint this repository declares; every arm present
    must belong to it — so a P2b cell relabelled ``"P2c"`` by hand is still
    refused, because ``smc_v1`` is not one of P2c's arms — and each cell's
    stated question must be the one its checkpoint asks. The control must also
    be present, because every delta in the report is taken against it.

    A cell that states no question at all is refused rather than guessed at. The
    generation that predated the field has been re-run, so no such artifact
    exists in this repository; recovering an identity from the arms would be
    code with nothing to run against, and a reader cannot tell a recovered
    identity from a stated one without being told.
    """
    missing = sorted(
        f"{c['information_set']} x {c['model']}"
        for c in cells
        if c["payload"].get("question") is None
    )
    if missing:
        raise ComparisonError(
            f"{missing} state no research question. Every cell records the question its "
            "checkpoint asks; a cell that does not was produced before that was true and "
            "has to be re-run rather than have an identity inferred for it."
        )
    ...
    stated = sorted({c["payload"]["question"] for c in cells})
