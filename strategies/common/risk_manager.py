...
class RiskAwareStrategy(IStrategy):
    ...
    def confirm_trade_entry(
        ...
        **kwargs: Any,
        ...
        decision = self.risk.evaluate_entry(
            ...
            data_age_s=self.data_age_seconds(pair, current_time),
            inference_age_s=self.inference_age_seconds(pair),
        )
        if not decision.allowed:
            logger.info("Entry on %s rejected by risk engine: %s", pair, decision.reason)
    ...
    def order_filled(
        ...
        try:
            if trade.is_open:
                self.risk.open_position(pair, float(trade.stake_amount or 0.0))
                return
    ...
    # ------------------------------------------------------------------
    # Overridable staleness hooks. Subclasses that consume market data or
