...
#:     opens; ``evaluate_entry`` vetoes until it expires.
...
#:     tightened campaign would leave behind.
...
#:
#: -- the worst case being the widest stop the band admits at the lowest leverage
#: ``position_size`` will divide by (1.0). For the committed campaign that floor
#: is ``1.0 * 0.15 = 0.15``. The value below is the one the demo path has always
#: used, in ``tools/demo_run.py``, ``tools/futures_dry_run.py`` and every carry
...
def sizing_cap_binds(limits: RiskLimits) -> bool:
    ...
    the binding constraint for any stop distance the band admits.
    """
    return limits.risk_per_trade_pct >= limits.max_position_pct * limits.max_stop_distance_pct
