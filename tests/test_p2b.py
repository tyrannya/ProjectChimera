...
def test_a_cell_that_states_no_research_question_is_refused_not_guessed_at():
    """The generation that predated the field has been re-run, so none exists.

    Inferring an identity from the arms would work for the arms `ohlcv14` is not
    alone in, and a reader could not tell a recovered identity from a stated one
    without being told. Refusing says which cell to re-run instead.
    """
    cells = [_fake_cell(arm, "xgboost", "P2b") for arm in (OHLCV14, SMC_V1)]
    del cells[1]["payload"]["question"]
    with pytest.raises(p2b_compare.ComparisonError, match="state no research question"):
        p2b_compare.checkpoint_of(cells)
