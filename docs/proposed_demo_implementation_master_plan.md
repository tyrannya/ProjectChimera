# HISTORICALLY ADOPTED: demo implementation master plan

Status: **ADOPTED 2026-09-03; SUPERSEDED FOR FUTURE SEQUENCING 2026-09-12 BY ROADMAP V3.**
This document remains authoritative as historical implementation provenance and
for already-adopted recorder semantics/amendments unless Roadmap v3 explicitly
replaces them. It originally expanded the adopted roadmap in
[`proposed_development_plan_post_fable_5_1_audit.md`](proposed_development_plan_post_fable_5_1_audit.md)
into an implementation-grade plan for the historical S0 to S6 sequence. The filename keeps the word `proposed` so that the audit
...
The historical S0→S6 next-task statements below are retained for provenance; active work now follows `proposed_futures_first_roadmap_v3.md`.
...
| A10 | 2026-09-06 | 7.2 | **The side-aware funding cost carried the wrong sign.** Section 7.2's `funding rate (entry)` row stated the cost as `-sign(side) * rate`, which contradicts that row's own prose ("veto an increase when the position would pay") and the already-frozen execution accounting convention `funding_cash_flow = -sign(side) * notional * rate`, in which a negative cash flow means the position paid. The positive cost rate is the negation of cash-flow-per-notional, so the cost is `sign(side) * rate` with `LONG = +1` and `SHORT = -1`, yielding the intended four cases: LONG pays a positive rate and receives a negative one, SHORT receives a positive rate and pays a negative one. The literal formula, read as written, vetoed exactly the increases the rule exists to permit. Adopted 2026-09-06 by owner decision, pre-evidence, and recorded in [`amendment_a10_funding_sign.md`](amendment_a10_funding_sign.md); this row is the mechanical consolidation that amendment reserved, and it is not a new amendment. Unchanged: `max_funding_cost_rate = 0.0005`, the veto condition and its purpose, every other Aegis rule, all recorder thresholds and semantics, the prospective recorder contract and its hash, `prospective_from = null`, and P4/P13 historical semantics and artifacts. This is a **pre-evidence sign-convention correction**, not a new scientific criterion: no model, statistic, economic result or live route is authorised by it. Made while `prospective_from` is `null`, before any acquisition and while no prospective minute existed. |
| A11 | 2026-09-06 | 3, 4.1 | **The named spot websocket base is reset by the egress network.** `wss://stream.binance.com:9443/ws`, named in the section 3 arrow table and the `spot.kline_1m` row of 4.1, is reset before any WebSocket frame is exchanged — confirmed network-layer rather than Binance-side by an identical reset probing an unrelated host on the same non-standard port (the same host on port 443 instead answers Binance's own `HTTP 451` restricted-location response, a separate and still-open matter this amendment does not address). `wss://data-stream.binance.vision:443/ws` is Binance's own documented market-data-only websocket base (`web-socket-streams.md`, official `binance/binance-spot-api-docs`) and was verified from a Full-network Claude Code Cloud environment to complete a genuine handshake, acknowledge a combined `SUBSCRIBE` to `btcusdt@kline_1m` and `btcusdt@bookTicker`, and deliver live frames of both in the exact documented spot payload shapes this recorder's parsers already read. Recorded in full at [`amendment_a11_spot_ws_transport.md`](amendment_a11_spot_ws_transport.md). This changes transport only, exactly as A3 did: no stream id, contract field, `contract_hash`, payload semantic, event/canonical timestamp semantic, coverage rule, or scientific criterion changes, and `prospective_from` remains `null`. Spot REST gap-fill (`api.binance.com`) and every USD-M endpoint (`fapi.binance.com`) are untouched and unasserted by this amendment. Made while `prospective_from` is `null`, before any acquisition and while no prospective minute existed. |
| A12 | 2026-09-07 | 6.5, 6.6 | **Measured slippage was debited twice for a fill model that prices it in.** Section 6.6's cash line reads `- slippage_paid`, while 6.5 defines slippage as "the difference between the fill price and the mid at decision" -- a measurement of a price, not a transfer -- and 5.2's `RecordedQuoteFillModel` fills at `ask * (1 + slippage)` / `bid * (1 - slippage)`, so the crossing AND the configured slippage are both already inside the executed price that `spot_notional_at_entry` is computed from. Subtracting `slippage_paid` as well charges the same money twice. The plan was genuinely ambiguous rather than wrong: 6.6's line is correct for the disjoint-terms model that the frozen `chimera.carry.accounting` arithmetic uses, where the modelled slippage is charged on an UN-slipped notional, and nothing in the adopted text said which of the two models it governed. For `RecordedQuoteFillModel` only: the crossing and the configured slippage are embedded in the executed fill price and therefore in the VWAP; inventory basis, margin and realised PnL are computed from that VWAP; measured slippage is accumulated and reported as evidence and is NOT debited from `free_cash`; fees remain separate explicit cash debits; funding remains its own term under A10. This must not be generalised to a future fill model carrying an economically distinct non-price slippage charge, which would need its own amendment. Adopted 2026-09-07 by owner decision, pre-evidence, and recorded in [`amendment_a12_recorded_quote_slippage.md`](amendment_a12_recorded_quote_slippage.md). Unchanged: every rule, parameter, threshold and veto; fee rates and their cash treatment; 6.5's definition and reporting of slippage; the prospective recorder contract and its hash; `prospective_from = null`; P4/P13 historical semantics and the frozen `chimera.carry.accounting` arithmetic. This is a **pre-evidence engineering-specification disambiguation**, not a new scientific criterion: no model, statistic, economic result or live route is authorised by it. Made while `prospective_from` is `null`, before any acquisition and while no prospective minute existed. |
...
- Slippage: the difference between the fill price and the mid at decision,
  per fill, per leg, reported in bps and in quote currency. **Amendment A12:**
  under `RecordedQuoteFillModel` this difference is already inside the executed
  price, so it is accumulated and reported as evidence and is not a cash debit.
...
free_cash        = capital - spot_notional_at_entry - fees_paid - slippage_paid + net_funding + realised_on_closed_legs
                   # - slippage_paid is superseded by amendment A12 for
                   # RecordedQuoteFillModel: the configured slippage and the
                   # bid/ask crossing are both inside the executed VWAP that
                   # spot_notional_at_entry is computed from, so subtracting the
                   # measured slippage as well charges it twice. It stays a
                   # reported quantity. The line as written above remains correct
                   # for a model that charges a non-price slippage cost on an
                   # un-slipped notional, which is what chimera.carry.accounting
                   # does. See docs/amendment_a12_recorded_quote_slippage.md.
