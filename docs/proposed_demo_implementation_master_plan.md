...
| A3 | 2026-09-04 | 14 | The S1 engineering row estimated disk at "about 1.5 GB per month raw gz plus 0.3 GB normalized at these streams". Measured on a real 44-minute BTCUSDT recording, the raw figure is low by a factor of about 28: **868.6 MiB of NDJSON compressing to 50.0 MiB at 17.4x**, at rates of 573.9 and 166.3 book updates per second, extrapolating to **roughly 43 GiB per month of gzipped raw** — of which the two `bookTicker` streams are over 99%. The normalized figure is an over-estimate and is left alone: the same measurement projects about 7 MiB per month. Corrected because a host provisioned on 1.5 GB fills within a day, and a full disk now halts the affected stream for the rest of the run, which would make the 30-day gate unreachable for a reason that has nothing to do with the market. No stream, sampling rate, threshold, contract field or scientific semantic is changed by this amendment; only the number a host is sized from. |
...
| PRECONDITIONS | S0 |
| ENGINEERING TASKS | PR-04, PR-05, PR-06; deploy on the host that will run the demo; supervisor; disk sizing — **amended A3, measured**: about **43 GiB per month of gzipped raw** at these streams (the two `bookTicker` streams are over 99% of it, at ~574 and ~166 updates per second, compressing 17.4x from 868.6 MiB of NDJSON measured over 44 minutes) plus about 7 MiB per month normalized; rates vary with market activity, so provision headroom. The first estimate of 1.5 GB per month raw gz was low by roughly 28x |
