...
each closed checkpoint concluded and did not conclude.

The filename keeps the word `proposed` because the audit that produced this
document links to it by that name and the audit is a historical record that is
not edited after the fact. The status line above, not the filename, says what
this document is.

Still true, and not changed by adoption: nothing here is a preregistration,
nothing here licenses a result, and nothing here changes any frozen verdict.
Adopting a roadmap authorises the *work*, never a conclusion.

Written 2026-09-03 against `main` at `177d4b60c7e137730ce88241b481941b07b4cd30`
and draft PR #67 at `36cdae48877b1d5fa88b2664c127b5307a917751`. Adopted on
`main` at `6ae7dffabd3251d243acb15d45bd115178173835`, the merge of PR #68.

## What adoption changed, exactly

| | |
| --- | --- |
| decision | the whole of section 0 is ADOPTED, as one decision |
| PR #68 | merged as the audit record; merge commit `6ae7dffabd3251d243acb15d45bd115178173835` |
| PR #67 (P14) | **closed without merging and without opening P14.** DESIGN DECLINED BEFORE OPENING / NO RESULT. Branch `claude/p14-native-tradeflow-prereg` retained at `36cdae48877b1d5fa88b2664c127b5307a917751` as historical design evidence |
| P8 | **WITHDRAWN AS MOOT / NOT OPENED.** Not failed, not negative |
| four outer blocks | no further **deciding** use. Descriptive context only |
| instrument | Binance USD-M BTCUSDT perpetual is the primary modelled and executed directional instrument; Binance spot BTCUSDT is the hedge leg, the price control and a supporting data source |
| next deciding science | prospective-first. S0 -> S1 -> S2 -> S3 -> S4 -> S5 -> S6, in this document and expanded in [`proposed_demo_implementation_master_plan.md`](proposed_demo_implementation_master_plan.md) |
| research budget | section 6 is authoritative and finite |
| unchanged | Aegis is the sole risk authority; `P4-HOLD` retired and unread; Styx sealed; P13 historical economics not run; no real money; no leverage above 1x; every negative result and every frozen artifact stands |

Adoption generated no scientific result. No statistic was computed, no model
was fitted, no artifact under `artifacts/` was touched, and no preregistration
hash moved.
...
**Made on 2026-09-03: ADOPTED in full, as one decision.** The seven items below
are the decision, kept in the words they were proposed in.
...
- [x] Owner decision recorded in `docs/current_development_plan.md` (ADOPTED),
      with the date, and at the head of this document.
- [x] PR #67 closed without merging and without opening P14, with the audit
      linked and the reasons recorded on the PR; branch
      `claude/p14-native-tradeflow-prereg` retained at
      `36cdae48877b1d5fa88b2664c127b5307a917751`.
- [x] P8 marked `withdrawn` and P14 marked `declined`; `nn/research_state.py`
      extended with both terminal states, with two-sided tests, and every
      front-door block regenerated.
- [x] Standing constraints updated with the finite budget of section 6 and the
      instrument change.
- [ ] S1 recorder branch opened as the first engineering task. **This is the
      next task: PR-04, the offline recorder contract, events, sink and
      normalizer, with no network.**
- [ ] S2 protocol drafted for independent review.

Nothing in S1 or S2 has started. No recorder code exists, no prospective datum
has been collected, and no rule has been frozen.
